...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var modalState = {}; // State який постійно модифікуеться за допомогою changeModalState(modalState);
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); // Створюемо, і розміщяемо блок зі статусом помилки

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status');
      item.appendChild(statusMessage); // FormData збирає всі дані з форми.

      var formData = new FormData(item); // Якщо це сама та форма. Розбираємо данні та записуемо ключ+значення у FormData
